...
export default class Addpoll extends React.Component {
  constructor(props) {
    super(props);
    this.state = { question: "", op1: "", op2: "", backgcolor: "",opcolor:""};
  }
  state = {
    loading: true,
    checked: false,
  ...
  _upload = () => {
    if (!this.state.question || !this.state.op1 || !this.state.op2 || !this.state.backgcolor || !this.state.opcolor) {
      ...
      <View style={styles.screen}>
        <Card style={styles.card}>
          ...
          <CardItem style={{ backgroundColor: '#0E043B', marginTop: screenHeight - 700, flexDirection: 'column' }}>
            ...
            <Item stackedLabel>
              <Label style={styles.fieldtitle} > TYPE background Color</Label>
            ...
            <Item stackedLabel>
              <Label style={styles.fieldtitle} > TYPE color for options</Label>
              <Input style={styles.fieldinput} placeholder="type colours other than background colour" onChangeText={(opcolor) => this.setState({ opcolor })} value={this.state.opcolor} ref='usn' returnKeyType='next' editable={true} />
            </Item>
