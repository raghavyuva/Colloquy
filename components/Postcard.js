...
const Postcard = ({ item, navigation, name, route }) => {
  ...
  let photo = `${Config.url}/${item?.postedBy?.userphoto.substring(
    8,
    item.postedBy.userphoto.length
  )}`;
  let photobody = `${Config.url}/${item?.photo?.substring(8, 38)}`;
      ...
      { text: "Cancel" },
      {
        ...
        onPress: () => {
          ...
          let fileUri =
            ...
            `${item.postedBy.username + randomstring}.jpg`;
  ...
  const Postcommentfollowers = (item) => {
    ...
    Image.getSize(photobody, (widthof, heightof) => {
      ...
      <TouchableOpacity
        onPress={() => {
          if (item.postedBy._id == user.user.user._id) {
            ...
            navigation.navigate("external", {
              screen: "userpro",
              params: { thread: item.postedBy._id },
            });
          }
        }}
      >
    ...
    >
      ...
      <View>
        ...
        >
          {item.photo && (
            <Image
              source={{ uri: photobody }}
              style={{
                width: name == "NormalView" ? width - 50 : imgwidth,
                height: name == "NormalView" ? 200 : imgheight,
                alignSelf: "center",
                resizeMode: name == "NormalView" ? "cover" : "contain",
                borderRadius: 10,
              }}
              indicator={Progress.Pie}
              indicatorProps={{
